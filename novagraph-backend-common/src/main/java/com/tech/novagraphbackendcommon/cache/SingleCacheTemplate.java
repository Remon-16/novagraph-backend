...
public class SingleCacheTemplate {
    /**
    ...
    public <R, E, V> V baseQuery(
        ...
        synchronized (lock) {
            ...
            try {
                // 4. 查数据库
                E entity = dbLoader.get();

                // 5. 转换 Entity -> VO
                V entityVo = converter.apply(entity);

                // 6. 写入缓存
                cacheSaver.accept(entityVo);
