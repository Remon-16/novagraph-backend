...
import com.tech.novagraphbackenduserservice.infrastructure.manager.handler.HisSPHandler;
...
@Slf4j
@Component
public class SyncSPHis2DBCompensatoryJob {
    ...
    @Resource
    private HisSPHandler hisSPHandler;

    @Scheduled(cron = "0 0 2 * * *")
    public void run() {
        log.info("HisSPHandler 开始补偿数据");
        Set<String> thumbKeys = redisTemplate.keys(
                UserCacheConstant.buildRedisKey(UserCacheConstant.getTempHisKey("")) + "*");
        Set<String> needHandleDataSet = new HashSet<>();
        thumbKeys.stream().filter(ObjUtil::isNotNull).forEach(thumbKey -> needHandleDataSet.add(
                thumbKey.replace(UserCacheConstant.TEMP_HIS_KEY_PREFIX, "")));

        if (CollUtil.isEmpty(needHandleDataSet)) {
            log.info("HisSPHandler 没有需要补偿的临时数据");
            return;
        }
        // 补偿数据
        for (String date : needHandleDataSet) {
            redisSyncTemplate.executeSync(hisSPHandler, date);
        }
        log.info("HisSPHandler 临时数据补偿完成");
    }
}
