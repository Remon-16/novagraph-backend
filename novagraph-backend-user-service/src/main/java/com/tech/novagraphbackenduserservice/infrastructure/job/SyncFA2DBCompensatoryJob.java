...
@Slf4j
@Component
public class SyncFA2DBCompensatoryJob {
    ...
    public void run() {
        log.info("UserFavoriteHandler 开始补偿数据");
        Set<String> thumbKeys = redisTemplate.keys(
                UserCacheConstant.buildRedisKey(UserCacheConstant.getTempUserFavoriteKey("")) + "*");
        Set<String> needHandleDataSet = new HashSet<>();
        thumbKeys.stream().filter(ObjUtil::isNotNull).forEach(thumbKey -> needHandleDataSet.add(
                thumbKey.replace(UserCacheConstant.TEMP_USER_FAVORITE_KEY_PREFIX, "")));

        if (CollUtil.isEmpty(needHandleDataSet)) {
            log.info("UserFavoriteHandler 没有需要补偿的临时数据");
            return;
        }
        // 补偿数据
        ...
        log.info("UserFavoriteHandler 临时数据补偿完成");
    }
}
